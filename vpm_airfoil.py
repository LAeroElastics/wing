    ...
    def generate(self):
        ...
        def camber_line(pts, max_camber, loc_max_camber):
            return numpy.where((pts >= 0) & (pts <= (1.0 * loc_max_camber)),
                               ...
                               max_camber * ((1.0 - pts) / numpy.power(1.0 - loc_max_camber, 2.0)) * (
                                       1.0 + pts - 2.0 * loc_max_camber))
        ...
        camber_dy = numpy.where((camber_divs >= 0) & (camber_divs <= self.loc_max_camber),
                                ((2.0 * self.max_camber) / numpy.power(self.loc_max_camber, 2.0)) * (
                                        self.loc_max_camber - camber_divs),
                                ((2.0 * self.max_camber) / numpy.power(1.0 - self.loc_max_camber, 2.0)) * (
                                        self.loc_max_camber - camber_divs))
        atan_dy = numpy.arctan(camber_dy)
        # キャンバーラインからの上下面高さ
        thickness = 5.0 * self.thickness * (
                0.2969 * (numpy.sqrt(camber_divs)) - 0.1260 * camber_divs - 0.3516 * numpy.power(camber_divs, 2.0) +
        ...
        for i_chord_iter, _ in enumerate(source_chord_points):
            for i_span_iter in range(num_span_max_iter):
                mesh_airfoil[i_chord_iter, i_span_iter, :] \
                    = [source_chord_points[i_chord_iter, 0], source_span_points[i_span_iter],
                       source_chord_points[i_chord_iter, 1]]

        for i_chord_iter, _ in enumerate(source_camber_points):
            for i_span_iter in range(num_span_max_iter):
                mesh_camber[i_chord_iter, i_span_iter, :] \
                    = [source_camber_points[i_chord_iter, 0], source_span_points[i_span_iter],
                       source_camber_points[i_chord_iter, 1]]
