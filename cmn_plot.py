...
def show2d(bodies, c_pts):
    ...
    pyplot.plot(bodies[:, 1, 0], bodies[:, 1, 2], ".", color="blue", ms=5, mew=5)
        ...
        #    pyplot.plot(annex.c_pts[:, i, 0], annex.c_pts[:, i, 2], "-", color="orange", ms=5, mew=0.5)
        #    pyplot.plot(body.mesh[:, i, 0], body.mesh[:, i, 2], ".", color="red", ms=5, mew=5)
        #    pyplot.plot(annex.c_pts[:, i, 0], annex.c_pts[:, i, 2], ".", color="green", ms=5, mew=0.5)
    ...
    for i_x in range(len(collocation[:, 0, 0])):
        for i_y in range(len(collocation[0, :, 0])):
            ax3d.quiver(collocation[i_x, i_y, 0], collocation[i_x, i_y, 1], collocation[i_x, i_y, 2],
                        0., 0., force[i_x, i_y, :]*hstack((0, 0, 0.0005)), color='k', linewidth=0.5)
