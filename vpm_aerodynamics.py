...
class Aerodynamics(object):
    ...
    def calc_local_vorticity(self,col_pts,src_pt1,src_pt2):
        temp1 = col_pts-src_pt1
        temp2 = col_pts-src_pt2
        norm1 = norm(temp1)
        norm2 = norm(temp2)

        return (norm1+norm2)*numpy.cross(temp1,temp2)\
               /(norm1*norm2*(norm1*norm2+temp1.dot(temp2)))
    ...
    def gen_aic_matrix(self,alpha,num_chord_max_iter,num_span_max_iter):
        ...
        i_matrix_x_iter = 0
        for i_element_y_iter in range(num_span_max_iter-1):
            for i_element_x_iter in range(num_chord_max_iter-1):
                i_matrix_y_iter = 0
                current_col_pt = collocation_pts[i_element_x_iter,i_element_y_iter,:]

                for i_colpts_y_iter in range(num_span_max_iter-1):
                    trailing_edge1 = boundary_pts[-1,i_colpts_y_iter+1,:]
                    trailing_edge2 = boundary_pts[-1,i_colpts_y_iter+0,:]
                    temp1 = current_col_pt-trailing_edge2
                    temp2 = current_col_pt-trailing_edge1
                    norm1 = norm(temp1)
                    norm2 = norm(temp2)

                    trailing_edge_vortex1 = numpy.cross(vinf,temp2)/(norm2*(norm2-vinf.dot(temp2)))
                    trailing_edge_vortex3 = numpy.cross(vinf,temp1)/(norm1*(norm1-vinf.dot(temp1)))
                    ...
                    for i_colpts_x_iter in range(num_chord_max_iter-1):
                        ...
                        partial_matrix[i_matrix_x_iter,i_matrix_y_iter,:] = trailing_edge_vortex+sum_vortex+sum_boundary_vortex
                        aic_matrix[i_matrix_x_iter,i_matrix_y_iter] \
                            = partial_matrix[i_matrix_x_iter,i_matrix_y_iter,0]*normal_vector[i_element_x_iter,i_element_y_iter,0]\
                              +partial_matrix[i_matrix_x_iter,i_matrix_y_iter,1]*normal_vector[i_element_x_iter,i_element_y_iter,1]\
                              +partial_matrix[i_matrix_x_iter,i_matrix_y_iter,2]*normal_vector[i_element_x_iter,i_element_y_iter,2]
